...
提取的事实（每行一条）："""
    ...
    def update_forgetting(self, current_time: Optional[float] = None):
        """执行艾宾浩斯遗忘更新"""
        if not self.memories:
            return []
...
if __name__ == "__main__":
    ...
    messages = [
        ("用户", "你好，我叫小林", "2026-06-20 10:00"),
        ("用户", "我喜欢打篮球", "2026-06-20 10:05"),
        ("用户", "我现在改为清淡饮食了", "2026-06-23 11:00"),
    ]

    mids = mb.add(messages)
    print(f"添加 {len(messages)} 条消息，生成 {len(mids)} 条记忆")

    # 检索
    results = mb.retrieve("我喜欢什么运动", top_k=3)
    print(f"检索结果: {len(results)} 条")
    for r in results:
        print(f"  → {r['content'][:40]}... (score={r['composite_score']})")

    # 测试状态过滤
    mb.update(mids[1], UpdateOperation.SUPERSEDE)  # "我喜欢打篮球" 被替代
    active_only = mb.retrieve("运动", filters={"status": "active"})
    print(f"Active 记忆: {len(active_only)} 条")
    ...
    # 测试 TF-IDF 基线
    print("\n[2] TFIDFMemoryBank 测试")
    tfidf = TFIDFMemoryBank()
    tfidf.add(messages)
    results = tfidf.retrieve("运动", top_k=3)
    print(f"TF-IDF 检索: {len(results)} 条")
    for r in results:
        print(f"  → {r['content'][:40]}... (score={r['composite_score']})")
