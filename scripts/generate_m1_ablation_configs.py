...
def _load_flat_yaml(path: Path) -> dict[str, Any]:
    ...
    for line_number, raw_line in enumerate(
        ...
        if not key:
            raise ValueError(
                f"{path}:{line_number}: empty configuration key"
            )
    ...
    if missing_fields:
        raise ValueError(
            f"{path}: missing required fields: {missing_fields}"
        )
...
def _serialise_scalar(value: Any) -> str:
    ...
    if isinstance(value, str):
        # JSON string syntax is also valid YAML.
...
def _write_flat_yaml(
    ...
    """Write a generated experiment configuration."""
    ...
    if missing_fields:
        raise ValueError(
            f"cannot write config with missing fields: "
            f"{missing_fields}"
        )

    lines = [
        "# Generated by scripts/generate_m1_ablation_configs.py.",
        "# Do not edit manually; update the generator instead.",
    ]
...
def build_expected_configs() -> dict[Path, dict[str, Any]]:
    ...
    for method_name, base_path in BASE_CONFIGS.items():
        ...
        if base_config["methods"] != method_name:
            raise ValueError(
                f"base config method mismatch in {base_path}: "
                f"expected {method_name!r}, "
                f"got {base_config['methods']!r}"
            )
...
def write_configs() -> int:
    ...
    for output_path, config in expected.items():
        _write_flat_yaml(output_path, config)

    print(
        f"Generated {len(expected)} M1 ablation configs in "
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Verify that generated configurations are "
