...
我以前住上海，现在搬到北京 -> previous_value=上海, value=北京
```
...
也可以使用 `messages_to_raw_messages()` 将简单消息列表转换为 `RawMessage`。
...
- 确认 `operation_hint` 是否完全满足 `versioning` 的需要；
...
- `operation_hint` 只是预处理阶段的提示，不是最终版本更新结果；
