...
class QueryType(Enum):
    """查询类型 —— 供 routing 模块使用"""
    CURRENT = "current"         # 问当前状态
    HISTORICAL = "historical"   # 问历史状态
    CHANGE = "change"           # 问状态变化
    GENERAL = "general"         # 通用知识，不需要个人记忆
...
if __name__ == "__main__":
    ...
    print(f"  类型: {example.memory_type.value}")
    print(f"  状态: {example.status.value}")
    print(f"  标签: {example.tags}")
    print(f"  有效: {example.is_active()}")

    print(f"\n序列化:")
    print(json.dumps(example.to_dict(), indent=2, ensure_ascii=False))

    print(f"\n核心接口:")
    print(f"  MemorySystem (抽象基类):")
    print(f"    - add(messages) -> List[str]")
    print(f"    - retrieve(query, top_k, filters) -> List[Dict]")
    print(f"    - update(memory_id, operation, **kwargs)")
    print(f"    - get(memory_id) -> MemoryPiece")
    print(f"    - delete(memory_id, soft=True)")
    print(f"    - save(filepath) / load(filepath)")

    print(f"\n子模块接口:")
    print(f"  VersionManager: classify_relationship() / build_version_chain()")
    print(f"  ViewManager:    get_current_view() / get_history_view()")
    print(f"  QueryRouter:    classify_query() / route()")