...
from statebudgetmem.baselines.memorybank.agents import MemoryAugmentedAgent, BaselineAgent
...
class DemoLLM:
    def __init__(self, api_key=None, mode="offline"):
        ...
        if mode == "online" and api_key:
            try:
                from statebudgetmem.baselines.memorybank.evaluator import DeepSeekLLM
...
def init_demo(api_key, mode_name):
    ...
    # 加载示例历史到 MemoryBank
    from statebudgetmem.baselines.memorybank.datasets import DEMO_HISTORY
...
def main() -> None:
    """Launch the full MemoryBank comparison demo."""
    demo.launch(share=False, server_name="0.0.0.0", server_port=7860)
